...
/// 아니다 — HWPX `pageBreak` 와 HWP5 break 비트는 break-before 속성이라 문단 시작의
/// 쪽 나눔은 그 문단 자신의 속성이고, 분할하면 원 서식을 물려받은 빈 문단이 남는다.
/// 분할 계약은 실제로 가를 자리(중간 오프셋)에서 잠근다. offset 0 계약은
/// `issue_7218_page_break_at_paragraph_start.rs` 가 맡는다.
#[test]
