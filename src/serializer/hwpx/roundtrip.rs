...
fn diff_paragraph_linesegs_with_axis(
    ...
    // [#5961] 교차 축 허용치는 없다.
    //
    // #5943 때는 원본 IR 이 HWP5 축, 재적재본이 HWPX 축이라 `textpos` 가 정당하게 달랐고,
    // 그 문단의 secd·cold 슬롯으로 설명되는 만큼만 봐주는 상한이 필요했다. 이제 HWPX
    // 파서가 읽을 때 HWP5 축으로 올리므로 **양쪽이 같은 축**이다. 허용치를 남겨두면
    // 진짜 드리프트를 가리므로 액면 그대로 비교한다.
